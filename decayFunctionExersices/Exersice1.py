...
for i,t0 in enumerate(t0s):
    ...
    plt.plot(t,y,ls=lst[i],lw=2,color=colors[i],
         label=r'$t_0$={}'.format(t0))
plt.legend(frameon=False,ncol=1,fontsize=size*2.5)
if save_figs:plt.savefig(path +'/expdecay_simple__NotLog_vs_t0.png',bbox_inches='tight')
plt.show()
...
plt.xticks(fontsize=2.5*size)
plt.yticks(fontsize=2.5*size)
plt.ylabel(r'$log(e^{-t/t0})$ ',fontsize=size*3)
plt.xlabel(r'$t$',fontsize=size*3)
t= np.arange(dt,maxt+dt,dt)
for i,t0 in enumerate(t0s):
    y = funcs.expDecay_simple(t, t0)
    plt.plot(t,np.log(y),ls=lst[i],lw=2,color=colors[i],
         label=r'$t_0$={}'.format(t0))
plt.legend(frameon=False,ncol=1,fontsize=size*2.5)
if save_figs:plt.savefig(path +'/expdecay_simple__Logy_vs_t0.png',bbox_inches='tight')
plt.show()