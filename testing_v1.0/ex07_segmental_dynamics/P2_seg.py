...
#way one
dyn = dict()
dyn['system'] = traj.Dynamics('P2',seg_t) # to compute the dynamics of the whole system I give seg_t, without any boolean data
# to distinguish the population we give the boolean data
dyn['train'] = traj.Dynamics('P2',seg_t,fseg['train'])
dyn['loop'] = traj.Dynamics('P2',seg_t,fseg['loop'])
dyn['tail'] = traj.Dynamics('P2',seg_t,fseg['tail'])
dyn['free'] = traj.Dynamics('P2',seg_t,fseg['free'])
#more compact way to to the same as the previoues 8 lines
#dyn = {k : traj.Dynamics('P1',seg_t,fs) for k,fs in fseg.items()}
#dyn['system'] = traj.Dynamics('P1',seg_t) 

##Notice!!! that we used short time data.
#On ex11 we will show how to wrap two trajectories with different time intervals
...
plt.ylabel(r'$P_2(t)$',fontsize=3*size)
...
plt.savefig('segvP2.png',bbox_inches='tight')
plt.show()

#save your data into pickle
if save_data_into_pickle:
    import pickle
    data_file ='my_segmental_dataP2.pickle'
