...
dp = obj.calc_density_profile(binl,dmax,mode='mass') # returns a dictionary containing the data in arrays
...
plt.savefig('mass_density.png',bbox_inches='tight')
plt.show()

dp = obj.calc_density_profile(binl,dmax,mode='number')
figsize=(3.3,3.3)
dpi=300
fig =plt.figure(figsize=figsize,dpi=dpi)
plt.minorticks_on()
...
plt.plot(dp['d'],dp['rho'],label=r'$\rho_{tot}$',color='green')
plt.legend(frameon=False)
plt.savefig('number_density.png',bbox_inches='tight')
plt.show()